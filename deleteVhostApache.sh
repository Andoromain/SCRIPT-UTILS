...
# =============================================================================
# Nom du script : deleteVhostApache.sh
# Description   : Outil de gestion pour supprimer des configurations Apache
# Auteur        : Ando Romain
# Version       : 1.0
# =============================================================================
...
function print_welcome() {
  clear
  echo -e "${BLUE}╔════════════════════════════════════════════╗${NC}"
  echo -e "${BLUE}║       GESTION DES VIRTUAL HOSTS APACHE     ║${NC}"
  echo -e "${BLUE}║              MODULE SUPPRESSION            ║${NC}"
  echo -e "${BLUE}╚════════════════════════════════════════════╝${NC}"
  echo -e "\n"
}

# Fonction pour lister les configurations Apache disponibles
function list_apache_configs() {
  echo -e "${BLUE}Configurations disponibles :${NC}"
  echo -e "${BLUE}───────────────────────────${NC}"
  
  local sites=($(ls -1 ${APACHE_AVAILABLE_DIR}/ 2>/dev/null | sort))
  ...
  for i in "${!sites[@]}"; do
    ...
    if [ -L "${APACHE_ENABLED_DIR}/${sites[$i]}" ]; then
...
function delete_virtual_host() {
  local config_name="$1"
  local sites_available="${APACHE_AVAILABLE_DIR}/$config_name"
  local sites_enabled="${APACHE_ENABLED_DIR}/$config_name"
  ...
  local server_name=$(grep -E "ServerName" "$sites_available" 2>/dev/null | head -1 | awk '{print $2}')
  ...
  # Recharger Apache
  echo -e "\n${BLUE}[SYSTÈME]${NC} Rechargement de la configuration Apache..."
  if systemctl reload apache2; then
    echo -e "${GREEN}[SUCCÈS]${NC} Virtual host '$config_name' supprimé avec succès."
  else
    echo -e "${RED}[ERREUR]${NC} Problème lors du rechargement d'Apache. Vérifiez manuellement."
  fi
}
...
function main() {
  ...
  local selected_config=$(ls -1 ${APACHE_AVAILABLE_DIR}/ | sort | sed -n "${choice}p")

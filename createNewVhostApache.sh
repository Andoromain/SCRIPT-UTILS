#!/bin/bash
# filepath: /home/user/SCRIPT/createNewVhostApache.sh

# =============================================================================
# Project Creation Script with Apache - v1.0
# Author: Ando Romain
# Description: Automated project setup with Apache virtual host configuration
# =============================================================================
...
check_dependencies() {
  ...
  required_modules=("rewrite" "proxy" "proxy_http")
  for module in "${required_modules[@]}"; do
    if ! apache2ctl -M 2>/dev/null | grep -q "$module"; then
      log_warning "Apache module '$module' might not be enabled. Virtual host functionality may be limited."
    fi
  done
}
...
print_banner() {
  ...
  cat << "EOF"
    _                     _            _____       _             
   / \   _ __   __ _  ___| |__   ___  |  ___|_   _| |_ _   _ ___ 
  / _ \ | '_ \ / _` |/ __| '_ \ / _ \ | |_  | | | | __| | | | __|
 / ___ \| |_) | (_| | (__| | | |  __/ |  _| | |_| | |_| |_| | |_ 
/_/   \_\ .__/ \__,_|\___|_| |_|\___| |_|    \__,_|\__|\__,_|\__|
        |_|                                                      
EOF
...
create_virtual_host_apache() {
  log_info "Création du virtual host pour $projectUrl ($projectType)"

  configFile="/etc/apache2/sites-available/$projectUrl.conf"
  ...
  log_info "Création du fichier de configuration Apache..."
...
<VirtualHost *:80>
    ...
    ErrorLog \${APACHE_LOG_DIR}/$projectUrl-error.log
    CustomLog \${APACHE_LOG_DIR}/$projectUrl-access.log combined
    ...
    <FilesMatch "\.php$">
        SetHandler "proxy:unix:/run/php/php$projectPhpVersion-fpm.sock|fcgi://localhost"
    </FilesMatch>
</VirtualHost>
EOF
    ;;
...
<VirtualHost *:80>
    ...
    ProxyRequests Off
    ProxyPass / http://localhost:$projectPort/
    ProxyPassReverse / http://localhost:$projectPort/
...
<VirtualHost *:80>
    ServerName $projectUrl
    DocumentRoot /var/www/$projectUrl/build
    
    <Directory /var/www/$projectUrl/build>
...
EOF
  ...
  *)
  ...
  if sudo apache2ctl configtest; then
    ...
    log_error "La configuration Apache contient des erreurs. Veuillez vérifier le fichier $configFile."
    return 1
  fi
}
...
create_summary() {
  ...
  echo -e "\n${GREEN}Configuration Apache:${NC} /etc/apache2/sites-available/$projectUrl.conf"
